...
export const StatsOverlay: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleSourceUpdate = (e: Event) => {
      const customEvent = e as CustomEvent<string>;
      setSourceUrl(customEvent.detail);
    };
    window.addEventListener('ag-player-source', handleSourceUpdate as EventListener);
    ...
    return () => {
      ...
      window.removeEventListener('ag-player-source', handleSourceUpdate as EventListener);
    };
  }, [isVisible]);
  ...
  useEffect(() => {
    if (memoryBarRef.current && memory) {
      const width = (memory.used / memory.total) * 100;
      memoryBarRef.current.style.setProperty('--width', `${width}%`);
    }
  }, [memory]);
    ...
    <div className="fixed bottom-4 right-4 z-[9999] pointer-events-none">
      <div className="bg-black/80 backdrop-blur-xl border border-white/10 rounded-xl p-3 shadow-2xl flex flex-col gap-2 min-w-[140px] animate-in slide-in-from-bottom-2 fade-in duration-300">
        ...
        <div className="flex justify-between items-center bg-white/5 px-2 py-1 rounded-md">
          ...
          <span
            className={`text-xs font-mono font-bold ${fps >= 55 ? 'text-emerald-400' : fps >= 30 ? 'text-yellow-400' : 'text-red-400'}`}
          >
            {fps}
          </span>
        </div>
        ...
        {memory && (
          <div className="flex flex-col gap-1.5 bg-white/5 px-2 py-2 rounded-md">
            ...
            <div className="w-full bg-zinc-800 h-1 rounded-full overflow-hidden">
              <div ref={memoryBarRef} className="h-full bg-emerald-500 transition-all duration-1000 dynamic-width" />
            </div>
          </div>
        )}
        ...
        {sourceUrl && (
          <div className="flex flex-col gap-1 bg-white/5 px-2 py-2 rounded-md overflow-hidden">
            ...
            <div
              className="text-[10px] text-zinc-300 font-mono break-all line-clamp-2 select-all cursor-pointer hover:text-emerald-400 transition-colors"
              onClick={() => {
        ...
        <div className="text-[9px] text-zinc-600 text-center mt-1 italic">Press Shift+D to hide</div>
      </div>
    </div>
  );
};
