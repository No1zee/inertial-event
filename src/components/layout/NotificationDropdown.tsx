...
export function NotificationDropdown() {
  const history = useLocalDataStore(state => state.watchHistory);
      ...
      try {
        ...
        if (history.length > 0) {
          ...
          const newEpPromises = watchedShows.map(async show => {
            ...
            const freshDetails = await contentApi.getDetails(show.contentId, typeForApi);
        ...
        <Menu.Items className="absolute right-0 mt-2 w-80 md:w-96 bg-[#141414] border border-white/10 rounded-xl shadow-2xl py-2 focus:outline-none z-50">
          ...
          <div className="max-h-[500px] overflow-y-auto custom-scrollbar">
            {loading ? (
              ...
              <div className="px-4 py-8 text-center text-zinc-500 text-sm">All caught up!</div>
            ) : (
              <>
      ...
      {({ active }) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <h4 className="font-bold text-sm text-zinc-200 truncate group-hover:text-white transition-colors">
              {item.title}
            </h4>
            <p className="text-xs text-zinc-500 mt-1 line-clamp-2">{item.description}</p>
            <div className="flex items-center gap-2 mt-2">
              <span className={cn('text-[10px] font-bold px-1.5 py-0.5 rounded', getBadgeStyle(label))}>{label}</span>
