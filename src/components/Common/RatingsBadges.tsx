      ...
      {ratings.rottenTomatoes && ratings.rottenTomatoes.score > 0 && (
        <div className="flex items-center gap-1.5">
          {ratings.rottenTomatoes.state === 'certified' && (
            ...
            <span className="text-lg" title="Certified Fresh">
              🍅
            </span>
          )}
          {ratings.rottenTomatoes.state === 'fresh' && (
            <span className="text-lg" title="Fresh">
              🍅
            </span>
          )}
          {ratings.rottenTomatoes.state === 'rotten' && (
            <span className="text-lg" title="Rotten">
              🦠
            </span>
          )}
          <span className="font-bold text-sm text-zinc-200">{ratings.rottenTomatoes.score}%</span>
        </div>
      )}
    </div>
  );
