...
export const parseHLSManifest = (manifest: string): HLSManifest => {
  ...
  const parseStreamInfo = (line: string): { bandwidth: number; resolution?: string } => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('#EXT-X-TARGETDURATION:')) {
      result.targetDuration = parseInt(line.replace('#EXT-X-TARGETDURATION:', ''));
    }

    if (line.startsWith('#EXT-X-MEDIA-SEQUENCE:')) {
      result.mediaSequence = parseInt(line.replace('#EXT-X-MEDIA-SEQUENCE:', ''));
    }
