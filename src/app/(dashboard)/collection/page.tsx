...
function UniverseContent() {
  ...
  const { contentState } = useLocalDataStore();
  ...
  const watchedCount = collection.parts.filter(p => !!contentState[String(p.id)]).length;
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="relative w-full h-[60vh] md:h-[70vh]">
        <div className="absolute inset-0">
          <Image
            src={getOptimizedImageUrl(collection.backdrop_path, 'original')}
            alt={collection.name}
            fill
            ...
            <div className="flex-1">
              <div className="flex justify-between text-xs text-muted-foreground mb-2">
                ...
                <span className={progress === 100 ? 'text-primary' : ''}>
                  {watchedCount}/{collection.parts.length} Watched
                </span>
              </div>
            ...
            >
              ...
              <div className="flex-1 pt-2 md:pt-0 space-y-2">
                <h3 className="text-xl md:text-3xl font-bold text-foreground flex items-center gap-3">
                  {movie.title}
                  {!!contentState[String(movie.id)] && <Check size={20} className="text-primary" />}
                </h3>
