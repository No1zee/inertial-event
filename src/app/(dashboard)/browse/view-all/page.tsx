...
export default function ViewAllPage() {
  ...
  const fetchContent = async ({ pageParam = 1 }) => {
    ...
    switch (railId) {
      case 'trending':
        return contentApi.getTrending(pageParam);
      case 'popular_tv':
        return contentApi.getPopularTV(pageParam);
      case 'scifi':
        return contentApi.getByGenre(878, 'movie', pageParam);
      case 'action':
      ...
      case 'romcom':
        return contentApi.discover({ with_genres: '10749,35', sort_by: 'popularity.desc', page: pageParam }, 'movie');
      case 'short':
        return contentApi.getShortAndSweet(pageParam);
      case 'feelgood':
        return contentApi.getFeelGood(pageParam);
      case 'horror':
        return contentApi.getByGenre(27, 'movie', pageParam);
      case 'anime':
        return contentApi.discover({ with_keywords: '210024', sort_by: 'popularity.desc', page: pageParam }, 'tv');
      case 'docu':
        return contentApi.getByGenre(99, 'movie', pageParam);

      // TV Mappings
      case 'day1':
        return contentApi.getDayOneDrops('tv', pageParam);
      case 'fresh':
        return contentApi.getFresh('tv', pageParam);
      case 'bangers':
        return contentApi.getBangers('tv', pageParam);
      case 'underrated':
        return contentApi.getUnderrated('tv', pageParam);
      case 'classics':
        return contentApi.getClassics('tv', pageParam);
      case 'drama':
        return contentApi.getByGenre(18, 'tv', pageParam);
      case 'comedy':
        return contentApi.getByGenre(35, 'tv', pageParam);
      case 'crime':
        return contentApi.getByGenre(80, 'tv', pageParam);
      case 'bg_scifi':
        return contentApi.getByGenre(10765, 'tv', pageParam);
      case 'reality':
        return contentApi.getByGenre(10764, 'tv', pageParam);

      case 'popular_movies':
        return contentApi.discover({ sort_by: 'popularity.desc', page: pageParam }, 'movie');
      case 'top_rated_movies':
        return contentApi.discover({ sort_by: 'vote_average.desc', 'vote_count.gte': 1000, page: pageParam }, 'movie');
      ...
      <div className="relative z-10 pt-32 md:pt-40 px-6 md:px-16 pb-32 max-w-[2400px] mx-auto">
        ...
        <div ref={ref} className="h-40 flex items-center justify-center mt-20">
          {isFetchingNextPage ? (
            ...
            <span className="text-zinc-600 text-xs uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
              Loading More
            </span>
          ) : items.length > 0 ? (
