...
class FallbackService {
  ...
  async resolveBestSource(
    content: { id?: string; _id?: string; title: string; type: 'movie' | 'tv' | 'anime' },
    _episode?: unknown
  ...
  handleSourceError(source: ResolvedSource): boolean {
    ...
    logger.error(
      'FallbackService',
      `Source ${source.provider} failed after ${this.MAX_RETRIES} retries. Triggering fallback.`
    );
