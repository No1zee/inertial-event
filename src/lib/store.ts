...
export const useLibraryStore = createWithEqualityFn<LibraryState>()(
  persist(
    (set, get) => ({
      library: [],
      history: [],
      ...
      finishDownload: (id, path) =>
        set(state => ({
          downloads: state.downloads.map(d => (d.id === id ? { ...d, status: 'completed', progress: 100, path } : d)),
        })),
      removeDownload: id =>
        set(state => ({
        ...
        set(state => {
          ...
          return {
            watchHistory: state.watchHistory.map(item =>
              item.id === id ? { ...item, currentTime, duration, progress, lastWatched: Date.now(), completed } : item
            ),
          };
        }),
