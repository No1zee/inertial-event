...
import { tmdbCircuit } from './circuitBreaker';
...
/**
 * Simple concurrency limiter to prevent TMDB rate limiting (429)
...
// Global TMDB concurrency limit - TMDB typically allows 40 requests per 10s
const tmdbLimit = pLimit(10);

const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

const fetchWithRetry = async (url: string, retries = MAX_RETRIES, options: any = {}): Promise<any> => {
  return tmdbCircuit.execute(async () => {
    try {
      ...
      if (retries > 0 && (isNetworkError || isTimeout || isRetryableStatus)) {
        ...
        console.warn(`[ContentAPI] Fetch failed (${error.message || 'Unknown Error'}), retrying in ${Math.round(delay)}ms... (${retries} left): ${url}`);
        await sleep(delay);
        return fetchWithRetry(url, retries - 1, options);
      }
      
      if (isNetworkError) {
        console.error(`[ContentAPI] Fatal Network Error: ${url}. Ensure connectivity and TMDB access.`);
      }
...
export const contentApi = {
  getCollectionDetails: async (
    ...
    try {
      const url = getTmdbUrl(`/collection/${collectionId}`, 'language=en-US');
      const res = await fetchWithRetry(url);
      const data = res.data;
      
      if (!data) throw new Error('Collection not found');

      const parts = (data.parts || []).map((item: any) => 
        transformToContent({ ...item, media_type: 'movie' })
      );
      ...
      return {
        ...
        parts: prioritizeContent(parts),
      };
    } catch (error) {
      console.warn(`[ContentAPI] Collection fetch failed for ID ${collectionId}:`, error);
      // Return empty fallback instead of mock to avoid confusing UI states
      return {
        id: typeof collectionId === 'string' ? parseInt(collectionId) : collectionId,
        name: 'Collection Not Found',
        overview: 'We could not find the details for this collection.',
        poster_path: '',
        backdrop_path: '',
        parts: [],
      };
  ...
  getTrending: async (page: number = 1): Promise<Content[]> => {
    ...
    try {
      ...
      try {
        ...
        if (data.length === 0) return generateMockContent(20);
  ...
  getShortsFeed: async (page: number = 1): Promise<Content[]> => {
    try {
      const url = getTmdbUrl('/trending/all/day', `language=en-US&page=${page}`);
      const res = await fetchWithRetry(url);
      const items = res.data.results || [];
      
      if (items.length === 0) return generateMockContent(15);

      // Fetch trailers in parallel for the discovery feed
      // Limit to 10 items for performance and context relevance
      const discoveryItems = items.slice(0, 10);
      
      const populated = await Promise.all(
        discoveryItems.map((item: TMDBItem) => tmdbLimit(async () => {
          try {
            const type = item.media_type || (item.first_air_date ? 'tv' : 'movie');
            const vidRes = await fetchWithRetry(getTmdbUrl(`/${type}/${item.id}/videos`, 'language=en-US'), MAX_RETRIES, { timeout: 3000 });
            return transformToContent({ ...item, videos: vidRes.data, type });
          } catch {
            return transformToContent(item);
          }
        }))
      );

      return prioritizeContent(populated);
    } catch (e) {
      console.error('[ContentAPI] Shorts Feed failed:', e);
  ...
  getUpcoming: async (page?: number): Promise<Content[]> => {
    try {
      const randomPage = page || 1;
      const urls = [
        getTmdbUrl('/movie/upcoming', `language=en-US&region=US&page=${randomPage}`),
        getTmdbUrl('/movie/upcoming', `language=en-US&region=US&page=${randomPage + 1}`)
      ];

      const responses = await Promise.allSettled(
        urls.map(url => tmdbLimit(() => fetchWithRetry(url)))
      );
      
      const data: TMDBItem[] = [];
      responses.forEach(res => {
        if (res.status === 'fulfilled' && res.value?.data?.results) {
          data.push(...res.value.data.results);
        }
      });
      ...
      const populated = await Promise.all(
        limited.map((item) => tmdbLimit(async () => {
          try {
            const vidRes = await fetchWithRetry(getTmdbUrl(`/movie/${item.id}/videos`, 'language=en-US'), MAX_RETRIES, { timeout: 5000 });
  ...
  getBangers: async (type: 'movie' | 'tv' | 'anime' | 'series' = 'movie', page?: number): Promise<Content[]> => {
    try {
      ...
      const urls = [
        getTmdbUrl(tmdbEndpoint, `sort_by=vote_average.desc&vote_count.gte=1000&language=en-US&page=${randomPage}${extraFilter}`),
        getTmdbUrl(tmdbEndpoint, `sort_by=vote_average.desc&vote_count.gte=1000&language=en-US&page=${randomPage + 1}${extraFilter}`)
      ];
  ...
  getClassics: async (type: 'movie' | 'tv' | 'anime' | 'series' = 'movie', page?: number): Promise<Content[]> => {
    try {
      ...
      const urls = [
        getTmdbUrl(tmdbEndpoint, `sort_by=popularity.desc&vote_average.gte=7.5&${dateFilter}&language=en-US&page=${randomPage}${extraFilter}`),
        getTmdbUrl(tmdbEndpoint, `sort_by=popularity.desc&vote_average.gte=7.5&${dateFilter}&language=en-US&page=${randomPage + 1}${extraFilter}`)
      ];
  ...
  getSimilar: async (id: string, type: 'movie' | 'tv' | 'anime' | 'series'): Promise<Content[]> => {
    try {
      ...
      const res = await fetchWithRetry(getTmdbUrl(`/${queryType}/${cleanId}/recommendations`, 'language=en-US&page=1'));
      const data = res.data.results || [];
      if (data.length === 0) return generateMockContent(12);
      return data.map((item: TMDBItem) => transformToContent({ ...item, type }));
    } catch {
  ...
  getViralAdultSwim: async (): Promise<Content[]> => {
    try {
      ...
      const results = await Promise.all(hits.map(id => tmdbLimit(() => fetchWithRetry(getTmdbUrl(`/tv/${id}`, 'language=en-US')))));
  ...
  getAdultSwimByGenre: async (genres: number[]): Promise<Content[]> => {
    try {
      ...
      // Secondary filter: Ensure we prioritize Network 80 and high-rated items
      const unique = results.filter((item, index, self) => index === self.findIndex(t => t.id === item.id));
      return prioritizeContent(unique.map((item: TMDBItem) => transformToContent({ ...item, type: 'tv' })));
    } catch {
      return generateMockContent(12);
    }
  },
  ...
  getAdultSwimAnime: async (): Promise<Content[]> => {
    try {
      // High-fidelity mature anime from Network 80 or tagged as Adult Swim
      const res = await fetchWithRetry(
        getTmdbUrl(
          '/discover/tv',
          'with_networks=80&with_genres=16&with_keywords=210024&vote_average.gte=7&first_air_date.gte=2000-01-01&sort_by=popularity.desc&language=en-US&page=1'
        )
      );
      const data = res.data.results || [];
      if (data.length === 0) {
        // Fallback to general mature anime if network 80 is too sparse
        return contentApi.getAnimeByGenre('with_genres=16&vote_average.gte=7.5');
      }
      return data.map((item: TMDBItem) => transformToContent({ ...item, type: 'anime' }));
    } catch {
      return generateMockContent(12);
    ...
    try {
      ...
      if (type === 'all') {
        const [movies, tv] = await Promise.all([
          tmdbLimit(() => fetchWithRetry(getTmdbUrl('/discover/movie', params))),
          tmdbLimit(() => fetchWithRetry(getTmdbUrl('/discover/tv', params.replace('primary_release_date', 'first_air_date')))),
        ]);
        const results = [
        ...
        const res = await fetchWithRetry(getTmdbUrl('/discover/movie', params));
  ...
  discover: async (
    ...
    try {
      ...
      const endpoint = type === 'movie' ? '/discover/movie' : '/discover/tv';
      const res = await fetchWithRetry(getTmdbUrl(endpoint, queryParams.toString()), MAX_RETRIES, { timeout: 10000 });
      const data = res.data.results || [];
      if (data.length === 0) return generateMockContent(10);
      return data.map((item: TMDBItem) => transformToContent({ ...item, type }));
    } catch (e) {
  ...
  getTrailer: async (id: string, type: 'movie' | 'tv' = 'movie'): Promise<string | null> => {
    try {
      ...
      const res = await fetchWithRetry(getTmdbUrl(`/${type}/${cleanId}/videos`, 'language=en-US'));
  ...
  getSeasonDetails: async (id: string | number, seasonNumber: number, type: 'movie' | 'tv' | 'anime' | 'series' = 'tv'): Promise<SeasonDetails | null> => {
    try {
      ...
      const cleanId = idStr.replace('tmdb_', '');
      const res = await fetchWithRetry(getTmdbUrl(`/tv/${cleanId}/season/${seasonNumber}`, 'language=en-US'));
      return res.data;
    } catch (e) {
  ...
  getRecommendations: async (id: string | number, type: 'movie' | 'tv' | 'anime' | 'series'): Promise<Content[]> => {
    ...
    try {
      const queryType = type === 'movie' ? 'movie' : 'tv';
      const res = await fetchWithRetry(getTmdbUrl(`/${queryType}/${tmdbId}/recommendations`, 'language=en-US'));
      const data = res.data.results || [];
  ...
  getPersonDetails: async (personId: number): Promise<TMDBPerson | null> => {
    try {
      const res = await fetchWithRetry(getTmdbUrl(`/person/${personId}`, 'language=en-US'));
      return res.data;
    } catch (e) {
  ...
  getPersonCredits: async (personId: number): Promise<Content[]> => {
    try {
      const res = await fetchWithRetry(getTmdbUrl(`/person/${personId}/combined_credits`, 'language=en-US'));
  ...
  getPersonalizedMix: async (trendingContent: Content[], preferences?: RecommendationPreferences): Promise<Content[]> => {
    ...
    try {
      ...
      const pools = await Promise.all([
        ...genrePoolPromises.map(p => tmdbLimit(() => p)),
        ...vibePoolPromises.map(p => tmdbLimit(() => p))
      ]);
  ...
  getReviews: async (id: string | number, type: 'movie' | 'tv' | 'anime' | 'series' = 'movie'): Promise<Array<{ id: string; author: string; content: string; url: string }>> => {
    try {
      ...
      const res = await fetchWithRetry(getTmdbUrl(`/${queryType}/${cleanId}/reviews`, 'language=en-US'));
...
export const prioritizeContent = (contents: Content[], preferences?: RecommendationPreferences): Content[] => {
  ...
  const filtered = contents.filter(c => {
    // Drop items that have absolutely no imagery to prevent placeholder spam
    if (!c.poster_path && !c.backdrop_path && (!c.poster || c.poster.includes('placeholder')) && (!c.backdrop || c.backdrop.includes('placeholder'))) {
      return false;
    }
...
const transformToContent = (item: TMDBItem, forcedType?: 'movie' | 'tv' | 'anime' | 'series'): Content => {
  ...
  return {
    ...
    backdrop: getOptimizedImageUrl(item.backdrop_path || item.backdropUrl, 'w780'),
    poster_path: item.poster_path || item.posterUrl || null,
    backdrop_path: item.backdrop_path || item.backdropUrl || null,
    ...
    type: (() => {
      ...
      if (rawType) {
        ...
        if (typeStr === 'movie' || typeStr === 'tv' || typeStr === 'anime' || typeStr === 'series') {
          return typeStr as 'movie' | 'tv' | 'anime' | 'series';
        }
      }
