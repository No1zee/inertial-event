...
// Helper to optimize mock content images
const optimizeMockItem = (item: Content): Content => ({
  ...item,
  poster: item.poster ? getOptimizedImageUrl(item.poster, 'poster') : item.poster,
  backdrop: item.backdrop ? getOptimizedImageUrl(item.backdrop, 'landscape') : item.backdrop,
});

// Mock data for testing when TMDB API isn't working
const RAW_MOVIES: Content[] = [
  {
    id: 'mock-1',
...
export const MOCK_MOVIES = RAW_MOVIES.map(optimizeMockItem);
export const MOCK_TV_SHOWS = RAW_TV_SHOWS.map(optimizeMockItem);
